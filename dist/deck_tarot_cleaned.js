...
var Deck = (function () {
  ...
  var _fontSize;
  function clampToScreen(x, y, cardWidth, cardHeight) {
    const padding = 10; // margen de seguridad opcional
...
var fan = {
  ...
  card: function card(_card6) {
    ...
    function getOffsets() {
      ...
      return {
        maxRotation: 120,
        radius: mobile ? 280 : 350,
        offsetX: mobile ? window.innerWidth / 4 : 0,
        offsetY: mobile ? -80 : -100
      };
    }
    ...
    _card6.fan = function (i, len, cb) {
      ...
      const angle = (i / (len - 1) * 120 - 60); // rango de -60° a +60°
      ...
      const { radius, offsetX, offsetY } = getOffsets();

      const xPos = -Math.cos(deg2rad(angle)) * radius * _fontSize / 16 + offsetX;
      const yPos = -Math.sin(deg2rad(angle)) * radius * _fontSize / 16 + offsetY;

      const clamped = clampToScreen(xPos + window.innerWidth / 2, yPos + window.innerHeight / 2, 112, 192);
  ...
  var bysuit = {
    ...
    card: function card(_card7) {
      ...
      _card7.bysuit = function (cb) {
        ...
        const spacingX = 140; // espacio horizontal entre cartas
        const spacingY = 250; // espacio vertical entre filas
        const columns = 11;
        ...
        const centerY = 0.5; // porque solo hay dos filas
